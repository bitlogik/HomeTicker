...
def printethprice():
    ...
    statsblkapi.getData()
    eth_price = float( priceeth_api.getKey("BTC_ETH/last") )
    myticker.clear_screen()
    myticker.write(("Ethereum Stats\r\n 1 ETH =  %.1f mBTC")% (eth_price*1000) )
    eth_price = float( priceeth_api.getKey("USDT_ETH/last") )
    time.sleep(3)
    myticker.pos_cursor(2,1)
    myticker.clear_line()
    myticker.write((" 1 ETH =  %.2f USD")% eth_price )
    blkethn = int(statsblkapi.getKey("result")[2:],16)
    time.sleep(3)
    myticker.pos_cursor(2,1)
    myticker.clear_line()
